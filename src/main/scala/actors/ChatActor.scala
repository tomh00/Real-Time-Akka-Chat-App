...
class ChatActor( roomName : String ) extends Actor with ActorLogging {
  /*
  ...
  * However an external database could be considered as the app scales
  ...
  private def handleJoinChat( join : JoinChat ) : Unit = {
    if ( ! usersInChat.contains( join.getUser ) ) {
      ...
      join.getUser.addChatActor( roomName, self )
...
object ChatActor {
  def props( roomName : String ) : Props = Props( new ChatActor( roomName ) )
}
